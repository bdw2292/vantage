...
def AssumePoltypeDefaults():
    ...
    os.chdir(SanitizePath(poltypejobpath))
    lastchar=poltypejobpath[-1]
    if lastchar!=r"\\":
        poltypejobpath+=r'\\'
    ...
    for f in files:
        if '.' in f and 'ttt' not in f:
            ...
            if suffix=='key':
                ...
                DefineMoleculeFilePath(poltypejobpath+f)
                LoadMolecule()
...
def ReadMoleculeFilePath():
    if os.path.isfile('moleculefilepath.txt'):
        temp=open('moleculefilepath.txt','r')
        ...
        print('Molecule file path is not defined')

def DefineMoleculeFilePath(filepath):
    tempname='moleculefilepath.txt'
...
def DefineParameterFilePath(filepath):
    tempname='parameterfilepath.txt'
    temp=open(tempname,'w')
    temp.write(filepath)
    temp.close()
...
def ReadParameterFilePath():
    if os.path.isfile('parameterfilepath.txt'):
        temp=open('parameterfilepath.txt','r')
        results=temp.readlines()
        temp.close()
        firstline=results[0]
        linesplit=firstline.split()
        ...
        print('Parameter file path has not been defined')
...
def DefineTinkerBINPath(binpath):
    ...
    if os.path.exists(binpath+'binpath.txt'):
        os.remove(binpath+'binpath.txt')
    temp=open(tempname,'w')
    temp.write(binpath)
    temp.close()
...
def GrabParameterFilePathFromTinkerKey():
    ...
    if prmfilepath==None:
        ...
        if not os.path.isdir(SanitizePath(prmfilepath)):
            tinkerpath=ReadTinkerBINPath()
            
def GrabParameterFilePath():
    prmfilepath=ReadParameterFilePath()
    if prmfilepath==None:
...
def GenerateTinkerTypeToTinkerClassDic():
    ...
    prmfilepath=GrabParameterFilePath()
...
def GenerateNewKeyName():
    ...
    os.chdir(SanitizePath(dir_path))
    files=os.listdir(os.getcwd())
    lastnumber=1
    for f in files:
        if 'key_' in f:
...
cmd.extend("DefineParameterFilePath",DefineParameterFilePath)
